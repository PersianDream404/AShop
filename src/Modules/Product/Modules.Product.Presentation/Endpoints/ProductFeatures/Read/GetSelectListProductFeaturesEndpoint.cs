...
public static class GetSelectListProductFeaturesEndpoint
{
    public class EndPoint : BaseEndpoint, IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet($"{ApiInfo.Prefix}/SelectList", handler: async (
                    ...
                    [AsParameters] GetSelectListProductFeaturesRequestDto request,
                  [FromServices] IQueryBus _queryBus
                ) =>
            {

                var result = await _queryBus.Send<GetSelectListProductFeaturesQuery, PagedList< GetSelectListProductFeaturesResponseDto >>
                                 (new GetSelectListProductFeaturesQuery(request));
