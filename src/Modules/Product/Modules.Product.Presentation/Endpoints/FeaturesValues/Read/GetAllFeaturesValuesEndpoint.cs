...
public static class GetAllFeaturesValuesEndpoint
{
    public class EndPoint : BaseEndpoint, IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet($"{ApiInfo.Prefix}", handler: async (
                    ...
                    [AsParameters] GetAllFeaturesValuesRequestDto request,
                  [FromServices] IQueryBus _queryBus
                ) =>
            {

                var result = await _queryBus.Send<GetAllFeaturesValuesQuery, PagedList< GetAllFeaturesValuesResponseDto >>
                                 (new GetAllFeaturesValuesQuery(request));
