...
public class PreparePaymentCommandHandler(IOrderCommandRepository commandRepository, IShoppingCartQueryRepository cartQueryRepository,IOrderQueryRepository orderQueryRepository)
    : ICommandHandler<PreparePaymentCommand, GetPreparePaymentResponseDto>
{
    public async Task<Result<GetPreparePaymentResponseDto>> Handle(PreparePaymentCommand command, CancellationToken cancellationToken)
    {
        try
        {
