...
public static class GetAllColorEndpoint
{
    public class EndPoint : BaseEndpoint, IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet($"{ApiInfo.Prefix}", handler: async (
                    ...
                    [AsParameters] GetAllColorRequestDto request,
                  [FromServices] IQueryBus _queryBus
                ) =>
            {

                var result = await _queryBus.Send<GetAllColorQuery, PagedList< GetAllColorResponseDto >>
                                 (new GetAllColorQuery(request));
